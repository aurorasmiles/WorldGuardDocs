...
The list of usable material names comes from the `Material list in Bukkit <http://jd.bukkit.org/rb/apidocs/org/bukkit/Material.html>`_. For example, the permission for placing the bed block would be ``worldguard.build.block.place.bed_block``. Be aware that *Material* contains both item and block names.
